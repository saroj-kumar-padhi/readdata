   ...
   return Scaffold(     
      body: Padding(
        ...
        child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('users_folder/folder/client_users').limit(limit).snapshots(),          
          builder: (context, snapshot) {
            if(snapshot.data!=null){                          
                snapshot.data!.docs.forEach((element) {
                  _allUsers.add({
                    'id':'${element.get('client_uid')}','name':'${element.get('client_name')}','age':element.get('client_age'),'location':element.get('client_location'),
                    'number':element.get('client_mobile_number')??'','email':element.get('client_email')??''
                  });
                });
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    }, icon: Icon(LineIcons.search), label: Text( location?"Search by name":"Search by location")),
                     SizedBox(width: 10,),
                    TextButton.icon(onPressed: (){
                      setState(() {
                        limit = limit+10;
                      ...
                      ? GridView.builder(
                          ...
                          itemBuilder: (context, index) => Card(
                            ...
                            child: ListTile(
                              ...
                              // ),
                              isThreeLine: true,
                              title: Text(_foundUsers[index]['name'],style: context.theme.textTheme.bodyMedium!.copyWith(fontWeight: FontWeight.bold,fontSize: 24),),
                              subtitle: Column(
